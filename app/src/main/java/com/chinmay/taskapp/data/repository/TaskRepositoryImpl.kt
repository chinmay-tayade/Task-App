...
class TaskRepositoryImpl(private val dao: TaskDao) : TaskRepository {
    ...
    override fun getAllTasks(): Flow<List<Task>> {
        return dao.getAllTasks().map { taskEntities ->
            taskEntities.map { Task(it.id, it.title, it.dueDate, it.status) }
        }
    }

    override suspend fun insertTask(task: Task) {
        dao.insertTask(TaskEntity(task.id, task.title, task.dueDate, task.status))
    }

    override suspend fun updateTask(task: Task) {
        dao.updateTask(TaskEntity(task.id, task.title, task.dueDate, task.status))
    }

    override suspend fun deleteTask(task: Task) {
        dao.deleteTask(TaskEntity(task.id, task.title, task.dueDate, task.status))
    }
}
